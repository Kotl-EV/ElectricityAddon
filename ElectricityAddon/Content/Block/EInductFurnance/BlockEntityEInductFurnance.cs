...
public class BlockEntityEInductFurnance : BlockEntityOpenableContainer, IHeatSource, ITexPositionSource
{
    ...
    GuiDialogBlockEntityEInductFurnance clientDialog;
    ...
    public virtual string DialogTitle => Lang.Get("BlockEInducFurnance");
    ...
    public BlockEntityEInductFurnance()
    {
    ...
    public override void Initialize(ICoreAPI api)
    {
        ...
        else
        {
            ...
            if (animUtil != null)
            {
                animUtil.InitializeAnimator("einducfurnance", null, null, new Vec3f(0, GetRotation(), 0f));
            }
        }
        UpdateMeshes();
        MarkDirty(true);
    ...
    private void OnBurnTick(float dt)
    {
        ...
        if (IsBurning)
        {
            stoveTemperature = changeTemperature(stoveTemperature, GetBehavior<BEBehaviorEInductFurnance>().powerSetting, dt);
        ...
        if (GetBehavior<BEBehaviorEInductFurnance>()?.powerSetting > 0)
        {
    ...
    public override void OnReceivedServerPacket(int packetid, byte[] data)
    {
        if (packetid == (int)EnumBlockStovePacket.OpenGUI)
        {
            using (MemoryStream ms = new MemoryStream(data))
            {
                ...
                {
                    clientDialog = new GuiDialogBlockEntityEInductFurnance(dialogTitle, Inventory, Pos, dtree, Api as ICoreClientAPI);
